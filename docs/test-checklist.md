...
| 142 | Tune slowly across a band in auto | It opens on each station and stays quiet between. It must not chatter on and off on a weak one, nor on the channel either side of a strong one, which is what 152a covers |
...
| 152a | In auto, tune to the channel beside the strongest station on the band, 102.0 next to 101.9 here | Quiet, and it stays quiet. Measured on 13 September 2026 after the change: open 0 of 30 readings with no change of state, against 23 of 25 with four changes before it. The shoulder passes the noise and multipath test because the sidebands of the station next door really are in the channel, so only level separates them |
| 152b | Do the same on the other side of a strong station, 91.0 next to 91.1 | The same. One shoulder is not a special case |
| 152c | From that shoulder, tune straight to a weak station | It opens within about a third of a second. Measured three times at 0.30, 0.31 and 0.41 s, which includes the HTTP round trip and the tuner settling. It was 0.17 to 0.25 s before opening needed two of the last three readings, so about 0.15 s of that is the cost of not opening on a single reading. The level average is also started again on every retune, without which this would be about a second |
| 152k | In auto on a station whose signal is drifting either side of the limit | It stays open rather than flickering, and if it is muted when you arrive it opens within a few readings. Opening needs two of the last three readings to be good, not two in a row: a signal sitting exactly on a limit alternates good and bad, so a run of two would never happen and the station would stay muted for ever. Staying open needs only one. Both halves of that have been got wrong once |
| 152l | In manual, set the threshold right at the station's level so it sits on the edge | It opens and stays open rather than going silent. Manual judges the raw level, which jumps several dB between readings on this tuner, so a threshold near the signal alternates by design |
| 152d | In auto, listen to the weakest station you can find | It stays open. The floor sits at 15.0 dBuV, below every station ever measured on this radio, the weakest being 19.6 dBuV in the seek sweep. Silence on a station you can hear is a worse fault than audio on a channel you did not want, and the floor errs that way on purpose |
| 152e | Read `sav` on a station the squelch is muting, not `sig` | The floor is judged on the smoothed level, so `sav` is the number to compare against the floor. If `sav` is above the floor and the audio is still muted, that is a fault worth a ticket. If it is below, the floor is doing what it was set to and the setting wants lowering rather than the code changing |
| 152h | Set the squelch floor to 0 on the Radio page, then to 40, then back | 0 leaves the squelch judging on noise, multipath and offset alone, which is what the old firmware does. 40 mutes most stations, which is how you can tell the setting reaches the squelch rather than only being stored: on a station reading 25.1 dBuV smoothed, 15 gave 15 of 15 readings open and 40 gave 0 of 15 |
| 152i | `POST /api/settings -d 'sqf=41'` | `400`. Every station measured here reads above 19.6 dBuV, so a floor past 40 would be asking the radio to mute the whole band |
| 152j | With the floor at its default, tune to a station that is genuinely weak and listen | It stays open. If it mutes, lower the floor rather than assuming the radio is broken, and say what `sav` read so the number can be remeasured |
| 152f | In auto on an AM band, tune to a weak but real station | It opens. The level floor is FM only, because it is fitted to FM captures on an FM level scale and there is no AM capture to fit an AM one to. Seek's floor is FM only for the same reason |
| 152g | Set seek sensitivity to 6, seek, and listen to what it stops on | It should not stop on something the squelch then mutes. Seek's floor falls to 4.0 dBuV at that setting, below the squelch's 15.0, so the two can in principle disagree. It has never been seen, because no station measured here has read below 19.6 dBuV. If it happens, say so on the ticket rather than nudging either number |
