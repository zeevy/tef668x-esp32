/**
 * @file backlight.h
 * @brief How bright the panel should be right now.
 *
...
/**
 * Fill a config in with what a panel gets when nobody has said otherwise.
 *
 * @param cfg  Receives the defaults.
 */
 ...
 *
 * @param s    The stored settings. NULL gives the defaults.
 * @param out  Receives the config.
 */
void backlightFromSettings(const Settings *s, BacklightConfig *out);

/**
 * Start the panel at nothing and begin the fade up.
 *
 * @param b      The backlight.
 * @param cfg    What it should do. NULL takes the defaults.
 ...
 * @param b      The backlight.
 * @param nowMs  Milliseconds since boot.
 * @return The brightness, 0 to 100.
 */
uint8_t backlightUpdate(Backlight *b, uint32_t nowMs);

/**
 * What the panel should be at, without moving anything.
 *
 * @param b  The backlight.
 * @return The brightness, 0 to 100. 0 for NULL.
 */
uint8_t backlightLevel(const Backlight *b);

/**
 * Whether the radio has been left alone long enough to have dimmed.
 *
 * @param b  The backlight.
 * @return true once the drop has started. False for NULL.
 */
 ...
 *
 * @param b  The backlight.
 * @return true while the level is still moving. False for NULL.
 */
